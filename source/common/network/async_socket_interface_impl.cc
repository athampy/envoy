#include "common/network/async_socket_interface_impl.h"
...
IoHandlePtr AsyncSocketInterfaceImpl::socket(Socket::Type socket_type,
  ...
  IoHandlePtr io_handle =
      AsyncSocketInterfaceImpl::socket(socket_type, addr->type(), ip_version, v6only);
  if (addr->type() == Address::Type::Ip && ip_version == Address::IpVersion::v6) {
...
AsyncSocketInterfaceImpl::createBootstrapExtension(const Protobuf::Message&,
                                                   Server::Configuration::ServerFactoryContext&) {
  return std::make_unique<SocketInterfaceExtension>(*this);
}

ProtobufTypes::MessagePtr AsyncSocketInterfaceImpl::createEmptyConfigProto() {
  return std::make_unique<
      envoy::extensions::network::socket_interface::v3::DefaultSocketInterface>();
}

REGISTER_FACTORY(AsyncSocketInterfaceImpl, Server::Configuration::BootstrapExtensionFactory);

static SocketInterfaceLoader* socket_interface_ =
    new SocketInterfaceLoader(std::make_unique<AsyncSocketInterfaceImpl>());
