...
load(":repository_locations.bzl", "DEPENDENCY_ANNOTATIONS",
     "DEPENDENCY_REPOSITORIES", "USE_CATEGORIES",
...
def _fail_missing_attribute(attr, key):
    fail("The '%s' attribute must be defined for external dependecy " % attr +
         key)
...
def _repository_locations():
    ...
    for key, location in DEPENDENCY_REPOSITORIES.items():
        ...
        if not project_url.startswith(
                "https://") and not project_url.startswith("http://"):
            fail("project_url must start with https:// or http://: " +
                 project_url)
        ...
        if len(last_updated
               ) != 10 or last_updated[4] != "-" or last_updated[7] != "-":
        ...
        if "cpe" in location:
            ...
            cpe_matches = (
                cpe != "N/A"
                and (not cpe.startswith("cpe:2.3:a:")
                     or not cpe.endswith(":*") and len(cpe.split(":")) != 6))
            if cpe_matches:
                fail("CPE must match cpe:2.3:a:<facet>:<facet>:*: " + cpe)
        elif not [
                category for category in USE_CATEGORIES_WITH_CPE_OPTIONAL
                if category in location["use_category"]
        ]:
            _fail_missing_attribute("cpe", key)

        for category in location["use_category"]:
            if category not in USE_CATEGORIES:
                fail("Unknown use_category value '" + category +
                     "' for dependecy " + key)
...
_default_envoy_build_config = repository_rule(
    ...
    attrs={
        "config":
        attr.label(
            default="@envoy//source/extensions:extensions_build_config.bzl"),
    },
)
...
def _com_github_c_ares_c_ares():
    ...
    http_archive(name="com_github_c_ares_c_ares",
...
def _com_github_google_benchmark():
    ...
    http_archive(name="com_github_google_benchmark", **location)
    native.bind(
        name="benchmark",
...
def _com_github_jbeder_yaml_cpp():
    _repository_impl(name="com_github_jbeder_yaml_cpp", )
    native.bind(
        name="yaml_cpp",
...
def _com_github_axboe_liburing():
    ...
    location = _get_location("com_github_axboe_liburing")
    print("_com_github_axboe_liburing ", location)
    http_archive(name="com_github_axboe_liburing",
                 build_file_content=BUILD_ALL_CONTENT,
                 **location)
    native.bind(
        name="liburing",
...
def _com_github_libevent_libevent():
    ...
    print("_com_github_libevent_libevent ", location)
    http_archive(name="com_github_libevent_libevent",
                 build_file_content=BUILD_ALL_CONTENT,
                 **location)
    native.bind(
        name="event",
...
def _com_github_datadog_dd_opentracing_cpp():
    ...
    _repository_impl(
        ...
        build_file=
        "@com_github_datadog_dd_opentracing_cpp//:bazel/external/msgpack.BUILD",
    )
    native.bind(
...
def _io_opencensus_cpp():
    ...
    http_archive(name="io_opencensus_cpp", **location)
    native.bind(
...
def _com_googlesource_googleurl():
    _repository_impl(name="com_googlesource_googleurl", )
    native.bind(
        name="googleurl",
...
def _com_github_grpc_grpc():
    ...
    native.bind(
        ...
        actual=
        "@com_github_grpc_grpc//test/core/tsi/alts/fake_handshaker:fake_handshaker_lib",
    )

    native.bind(
        name="grpc_alts_handshaker_proto",
        actual=
        "@com_github_grpc_grpc//test/core/tsi/alts/fake_handshaker:handshaker_proto",
    )

    native.bind(
        name="grpc_alts_transport_security_common_proto",
        actual=
        "@com_github_grpc_grpc//test/core/tsi/alts/fake_handshaker:transport_security_common_proto",
    )
...
def _com_github_luajit_luajit():
    ...
    http_archive(name="com_github_luajit_luajit",
                 build_file_content=BUILD_ALL_CONTENT,
...
def _com_github_moonjit_moonjit():
    ...
    http_archive(name="com_github_moonjit_moonjit",
                 build_file_content=BUILD_ALL_CONTENT,
...
def _com_github_gperftools_gperftools():
    ...
    http_archive(name="com_github_gperftools_gperftools",
                 build_file_content=BUILD_ALL_CONTENT,
                 **location)
...
filegroup(
    ...
    """
    http_archive(name="kafka_source",
    ...
    # tests.
    http_archive(name="kafka_server_binary",
                 build_file_content=BUILD_ALL_CONTENT,
    ...
    http_archive(name="kafka_python_client",
                 build_file_content=BUILD_ALL_CONTENT,
